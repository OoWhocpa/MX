...
function playVideo() {
	...
	video.addEventListener('timeupdate', function () {
		...
		// Показуємо коментарі після 960 секунд
		if (videoTime >= 960 && commentsPaused) {
...
document.querySelectorAll('.enter_page').forEach(e => {
	e.addEventListener('click', () => {
		document.querySelector('.basic_body').classList.remove('d-none')
		document.querySelector('.preload_page').classList.add('d-none')
